...
$imgurl = $s2->upload('img',$name,$_FILES['myfile']['tmp_name']);//把用户传到SAE的文件转存到名为test的storage
...
echo $face_id;
    $response = $facepp->execute('/person/create',array('face_id'=>$face_id,'person_name'=>$person_name,'group_name'=>$group_name));
    print_r($response);
?>