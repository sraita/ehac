...
$sql = mysql_query("SELECT device.data, device.sid, device.nid, device.devicename, device.id, device.status,
						ehac_user.openid, ehac_user.nickname, ehac_user.email, ehac_user.temp_device, ehac_user.temp_l, ehac_user.temp_h, ehac_user.humidity_device, ehac_user.humidity_l, ehac_user.humidity_h, ehac_user.alarm
...
{		//湿度低于
        ...
        {	
            ...
            if($arr['humidity_device']==$arr['id']){
            mysql_query("UPDATE device SET data='1' WHERE openid='".$arr['openid']."' AND id='".$arr['id']."';");
            }
            //邮件通知
    ...
    //湿度高于
        if($arr['sid']=1&&$arr['data']!=null && $arr['humidity_h']<substr($arr['data'],0,2))
        {
            $humidity = substr($arr['data'],0,2);
            if($arr['humidity_device']==$arr['id']){
            mysql_query("UPDATE device SET data='0' WHERE openid='".$arr['openid']."' AND id='".$arr['id']."';");
            }
            //邮件通知
    ...
    //温度低于
    	if($arr['sid']=1&&$arr['data']!=null && $arr['temp_l']>substr($arr['data'],3))
        {
            $temp = substr($arr['data'],3);
            if($arr['temp_device']==$arr['id']){
            mysql_query("UPDATE device SET data='0' WHERE openid='".$arr['openid']."' AND id='".$arr['id']."';");
            }
            //邮件通知
    ...
    //烟雾预警值超限
        ...
        {	
            mysql_query("UPDATE device SET data='0' WHERE openid='".$arr['openid']."' AND sid='002';");
            //邮件通知
    ...
    //室内环境异常
        if($arr['sid']=4&& $arr['data']=1)
        {	
            //邮件通知
            $message = "您好".$arr['nickname']."！室内环境异常，请及时查看！";
            send_mail_lazypeople($arr['email'],'环境异常通知',$message);//测试
            //模板消息-烟雾预警通知
        	$template_alarm = '{
            "touser": "'.$arr['openid'].'", 
